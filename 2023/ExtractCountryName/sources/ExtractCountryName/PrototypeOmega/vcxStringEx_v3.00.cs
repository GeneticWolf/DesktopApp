...
namespace PrototypeOmega {
	internal static class StringEx {
		internal static bool AttemptSetTextClipBoard(string pstrText = "", int plngNbAttempt = 1, bool pblnWarningOnFail = false) {
			...
			if (plngNbAttempt > 0) {
				...
				do {
					...
					if (pstrText.Length == 0) {
						...
						strVerb = "[SET]";
						ClipboardEx.SetText(pstrText);
					}

					strTest = ClipboardEx.GetText();
					if (strTest == pstrText) {
						blnSuccess = true;
						break;
					} else {
						if (pblnWarningOnFail == true) {
							string strMessage = "Program was unable to " + strVerb + " clipboard text and will";
							int lngRemain = (plngNbAttempt - lngAttempt);
							if (lngRemain > 1) {
								strMessage = strMessage + " reattempt " + lngRemain.ToString() + " times";
							} else if (lngRemain == 1) {
								strMessage = strMessage + " reattempt " + lngRemain.ToString() + " time";
							} else {
								strMessage = strMessage + " not reattempt";
							}

							AppEx.MessageBoxEx(strMessage);
						}
					}

					AppEx.Sleep(250);
				} while (lngAttempt < plngNbAttempt);
			}
